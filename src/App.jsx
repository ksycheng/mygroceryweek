...
export default function App() {
  ...
  const findPrices = async () => {
    ...
    try {
      // CALL 1: Search the web for prices across all stores
      setLoadingStep(1);
      const searchRes = await fetch("/api/prices", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ mode: "search", items: items.map(i => i.name), postal, budget }),
      });
      const searchData = await searchRes.json();
      if (searchData.error) throw new Error(searchData.error);

      // CALL 2: AI reads search results and extracts prices + builds comparison
      setLoadingStep(2);
