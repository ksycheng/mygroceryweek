...
const css = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .member-chip { display:inline-flex; align-items:center; gap:6px; padding:6px 12px; border-radius:99px; background:#e8f5e0; color:#2d5a1b; font-size:13px; font-weight:500; margin:4px; }
...
export default function App() {
  ...
  const checkPendingInvites = async (userId) => {
    ...
    if (!email) return;
    const { data } = await supabase.from("household_invites")
  ...
  const createHousehold = async () => {
    ...
    try {
      ...
      if (hhErr) { console.error("Household error:", hhErr); alert("Error creating household: " + hhErr.message); return; }
      if (hh) {
        setHousehold(hh);
        if (listId) {
          await supabase.from("grocery_lists").update({ household_id: hh.id }).eq("id", listId);
        } else {
    ...
    } catch (err) { console.error("createHousehold error:", err); }
  };
  ...
  const acceptInvite = async (invite) => {
    ...
    const { data } = await supabase.from("grocery_lists").select("*").eq("household_id", invite.household_id).single();
  ...
  const checkWishlistSales = async () => {
    ...
    const city = profile.postal.slice(0,3);
    const cityMap = { "L6C":"Markham","L6B":"Markham","L6E":"Markham","M1B":"Scarborough","M1P":"Scarborough","M4A":"East York","M5A":"Toronto","M5V":"Toronto","L4B":"Richmond Hill","L4C":"Richmond Hill","L4J":"Thornhill","L5A":"Mississauga","L5B":"Mississauga","L6P":"Brampton","L6R":"Brampton" };
    const cityName = cityMap[city] || "Toronto";
    try {
      const newResults = {};
      for (const item of wishlist) {
        const res = await fetch("/api/prices", {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            system: "You are a Canadian grocery deal finder. Return ONLY valid JSON.",
            prompt: "Search for current sale price of \"" + item.name + "\" at grocery stores near " + cityName + " Ontario Canada. Is it on sale this week? Return ONLY this JSON: {"currentPrice": 0.00, "regularPrice": 0.00, "onSale": false, "saleStore": \"Store name\", "saleEnds": \"date or unknown\", "savings": 0.00, "note": \"brief note about the deal\"}"
          }),
        });
        const data = await res.json();
        const text = (data.text || "").replace(/```json|```/g,"").replace(/\n/g," ").trim();
        const match = text.match(/{[sS]*}/);
        if (match) {
          const parsed = JSON.parse(match[0]);
          newResults[item.id] = parsed;
          const isOnSale = parsed.onSale && (!item.target_price || parsed.currentPrice <= item.target_price);
  ...
  const handleLogout = async () => {
    ...
    setItems([]); setResults(null); setHistory([]); setProfile(null); setHousehold(null); setHouseholdMembers([]);
  };
  ...
  const loadDishes = async () => {
    ...
    try {
      const system = "You are a professional chef and nutritionist. Return ONLY valid JSON, no markdown, no code blocks.";
      const prompt = "Suggest 8 dishes for " + profile.people + " people who enjoy " + (profile.cuisines||[]).join(", ") + " cuisine. Include a mix of Breakfast, Lunch, Dinner, Snack, and Dessert categories. Return ONLY this JSON array: [{\"name\":\"Dish Name\",\"emoji\":\"🍝\",\"mealCategory\":\"Dinner\",\"cuisine\":\"Italian\",\"prepTime\":\"15 mins\",\"cookTime\":\"30 mins\",\"totalTime\":\"45 mins\",\"difficulty\":\"Easy\",\"servings\":" + profile.people + ",\"costPerPersonCAD\":4.50,\"description\":\"One sentence\",\"nutrition\":{\"calories\":450,\"protein\":\"32g\",\"carbs\":\"48g\",\"fat\":\"12g\"},\"tips\":[\"Helpful tip 1\",\"Helpful tip 2\"],\"ingredients\":[{\"name\":\"ingredient\",\"amount\":\"2\",\"unit\":\"cups\",\"notes\":\"optional note\"}],\"steps\":[{\"title\":\"Step title\",\"detail\":\"Detailed step-by-step instruction with technique, temperature, and timing\"}]}]. mealCategory must be one of: Breakfast, Lunch, Dinner, Snack, Dessert. costPerPersonCAD is estimated cost in Canadian dollars per person.";
      const text = await callAI(system, prompt);
      const clean = text.replace(/```json|```/g, "").trim();
      const match = clean.match(/\[[\s\S]*\]/);
  ...
  const addItem = () => {
    ...
    const updated = [...items, { id: Date.now(), name, category: categorize(t), qty }];
    setItems(updated); saveList(updated);
  ...
  const findPrices = async () => {
    ...
    try {
      // Use new web search pricing engine
      const res = await fetch("/api/prices", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const saveWeek = async () => {
    ...
    if (!error) { loadHistory(user.id); alert("Week saved! Using: " + chosen?.label); }
  };
  ...
  const scanReceipt = async (file) => {
    ...
    try {
      ...
      const response = await fetch("/api/prices", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          system: "You are a receipt scanner. Extract purchased items from this receipt image. Return ONLY a JSON array of item name strings that appear on the receipt. No markdown.",
          prompt: "Extract all grocery item names from this receipt. Return ONLY a JSON array of strings like: [\"milk\", \"bread\"]. Receipt image (base64): " + base64.substring(0, 500) + "..."
        }),
      });
      const data = await response.json();
      const text = data.text || "";
      const match = text.match(/[[sS]*]/);
      if (!match) throw new Error("Could not read receipt");
      const purchasedItems = JSON.parse(match[0]).map(s => s.toLowerCase().trim());
      const newChecked = { ...checked };
      items.forEach(item => {
        const itemName = item.name.toLowerCase();
        const found = purchasedItems.some(p =>
          itemName.includes(p) || p.includes(itemName.split(" ")[0])
        );
        if (found) newChecked[item.id] = true;
      });
      setChecked(newChecked);
      const crossedCount = Object.values(newChecked).filter(Boolean).length;
      alert("Receipt scanned! " + crossedCount + " items matched and crossed off.");
    } catch (e) {
      setReceiptError("Could not scan receipt. Try a clearer photo.");
    } finally {
  ...
  const dishesByCategory = filteredDishes.reduce((acc, dish) => {
    const cat = dish.mealCategory || "Dinner";
    acc[cat] = acc[cat] || [];
    acc[cat].push(dish);
    return acc;
  }, {});
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#f7f4ef", fontFamily:"'DM Sans',sans-serif", color:"#1a1a1a" }}>
        ...
        <div style={{ maxWidth:640, margin:"0 auto" }}>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16 }}>
            ...
            <div style={{ display:"flex", gap:8, alignItems:"center" }}>
              ...
              <button onClick={()=>setShowFamily(f=>!f)} style={{ background:showFamily?"#e8f5e0":"none", border:"1px solid #4a7a30", borderRadius:8, padding:"7px 12px", fontSize:12, color:showFamily?"#2d5a1b":"#8fa87a", cursor:"pointer", position:"relative" }}>👨‍👩‍👧{householdMembers.length>0&&<span style={{ position:"absolute", top:-4, right:-4, background:"#3d8c23", color:"#fff", borderRadius:99, fontSize:9, padding:"1px 5px", fontWeight:700 }}>{householdMembers.length}</span>}</button>
          ...
          <div style={{ display:"flex" }}>
            {[["list","📋 List"],["meals","🍽️ Meals"],["compare","📊 Compare"],["wishlist","⭐ Wishlist"],["history","📈 History"]].map(([t,l])=>(
      ...
      <div style={{ maxWidth:640, margin:"0 auto", padding:"20px 16px 48px" }} className="fade-in">
        ...
        {showFamily&&(
          <div style={{ position:"fixed", inset:0, zIndex:100, display:"flex", alignItems:"flex-end", justifyContent:"center" }} onClick={e=>e.target===e.currentTarget&&setShowFamily(false)}>
            <div className="fade-in" style={{ background:"#fff", borderRadius:"20px 20px 0 0", padding:24, width:"100%", maxWidth:640, boxShadow:"0 -8px 40px rgba(0,0,0,0.25)", maxHeight:"80vh", overflowY:"auto" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
                <div>
                  ...
                  <p style={{ fontSize:13, color:"#7a7060", margin:0, marginTop:2 }}>Share your grocery list in real time</p>
                </div>
                <button onClick={()=>setShowFamily(false)} style={{ background:"#f0f0f0", border:"none", borderRadius:99, width:32, height:32, fontSize:18, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>×</button>
              </div>
              {!household?(
                <div>
                ...
                <div>
                  ...
                  {householdMembers.length > 0 && (
                    <div style={{ marginBottom:16 }}>
                      <p style={{ fontSize:12, fontWeight:600, color:"#7a7060", textTransform:"uppercase", letterSpacing:"0.5px", marginBottom:8 }}>Members</p>
                      <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
                        {householdMembers.map((m,i)=>(
                          <span key={i} style={{ display:"inline-flex", alignItems:"center", gap:6, padding:"8px 14px", borderRadius:99, background:"#e8f5e0", color:"#2d5a1b", fontSize:13, fontWeight:600 }}>
                  ...
                  {inviteStatus==="error"&&<div style={{ background:"#fde8e8", borderRadius:10, padding:"10px 14px", fontSize:13, color:"#c0392b" }}>❌ Failed to send invite. Try again.</div>}
                </div>
              )}
          ...
          <div>
            ...
            <div className="card" style={{ padding:16, marginBottom:14 }}>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                ...
                <button onClick={()=>setItemQty(q=>String(Math.max(1,parseInt(q)||1)-1))} style={{ background:"#f0f0f0", border:"none", borderRadius:6, padding:"4px 10px", cursor:"pointer", fontWeight:700 }}>-</button>
                <span style={{ fontSize:15, fontWeight:700, minWidth:20, textAlign:"center" }}>{itemQty}</span>
                <button onClick={()=>setItemQty(q=>String((parseInt(q)||1)+1))} style={{ background:"#f0f0f0", border:"none", borderRadius:6, padding:"4px 10px", cursor:"pointer", fontWeight:700 }}>+</button>
                <span style={{ fontSize:12, color:"#aaa", marginLeft:8 }}>Tap + to add multiple of same item</span>
              </div>
            </div>
            ...
            {items.length>0&&(
              <div style={{ display:"flex", flexDirection:"column", gap:10, marginTop:8 }}>
                ...
                <input ref={receiptInputRef} type="file" accept="image/*" capture="environment" style={{ display:"none" }} onChange={e=>e.target.files[0]&&scanReceipt(e.target.files[0])} />
                <button onClick={()=>receiptInputRef.current?.click()} disabled={receiptScanning} style={{ background:"none", border:"1.5px solid #3a6fd8", borderRadius:10, padding:12, fontSize:13, color:"#3a6fd8", cursor:"pointer" }}>
                  {receiptScanning?"📷 Scanning receipt...":"📷 Scan Receipt to Auto-Tick Items"}
                </button>
                {receiptError&&<p style={{ fontSize:12, color:"#c0392b" }}>⚠️ {receiptError}</p>}
              ...
              <div className="fade-in">
                ...
                <div className="card" style={{ overflow:"hidden", marginBottom:16 }}>
                  ...
                  <div style={{ padding:20 }}>
                    ...
                    <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:8, marginBottom:14 }}>
                      <div style={{ background:"#f7f4ef", borderRadius:10, padding:"10px", textAlign:"center" }}>
                    ...
                    {selectedDish.prepTime&&(
                      <div style={{ display:"flex", gap:16, fontSize:12, color:"#7a7060", marginTop:8 }}>
                        <span>🥄 Prep: {selectedDish.prepTime}</span>
                        <span>🍳 Cook: {selectedDish.cookTime}</span>
                      </div>
                    )}
                  </div>
                </div>
                ...
                {selectedDish.tips&&selectedDish.tips.length>0&&(
                  <div className="card" style={{ padding:16, marginBottom:16, background:"#fef9f0", border:"1px solid #f0e0b0" }}>
                    ...
                    {selectedDish.tips.map((tip,i)=>(
                      <p key={i} style={{ fontSize:13, color:"#7a7060", marginBottom:i<selectedDish.tips.length-1?8:0, paddingLeft:12, borderLeft:"2px solid #d4870a" }}>{tip}</p>
                    ))}
                  </div>
                )}
                  ...
                  Object.entries(dishesByCategory).map(([cat, catDishes])=>{
                    ...
                    return (
                      <div key={cat}>
                        ...
                        {catDishes.map((dish,i)=>(<DishCard key={i} dish={dish} onClick={()=>{setSelectedDish(dish);setHaveIngredients({});}} />))}
                      </div>
                    );
                  })
                ):(
                  filteredDishes.length===0
                    ? <div style={{ textAlign:"center", padding:"32px", color:"#bbb" }}><p>No {activeMealCat} dishes yet. Click Refresh to generate more.</p></div>
                    : filteredDishes.map((dish,i)=>(<DishCard key={i} dish={dish} onClick={()=>{setSelectedDish(dish);setHaveIngredients({});}} />))
                )}
          ...
          <div>
            ...
            {results&&(
              <div>
                ...
                <p style={{ fontSize:12, color:"#aaa", marginBottom:12 }}>🔍 Prices from live web search. Tap a strategy to select it for Save Week. {results.priceNote || ""}</p>
                {results.combinations&&results.combinations.map((combo,i)=>(
                  <div key={i} onClick={()=>{ setSelectedCombo(i); setWeekSpend(combo.totalCAD); }} className="card" style={{ marginBottom:14, overflow:"hidden", border:selectedCombo===i?"2px solid #3d8c23":"1px solid #e2dbd0", cursor:"pointer", transform:selectedCombo===i?"scale(1.01)":"none", transition:"all 0.2s" }}>
                    ...
                    {/* Store details with address, distance, hours */}
          ...
          <div className="fade-in">
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
              <div>
                ...
                <p style={{ fontSize:13, color:"#7a7060", marginTop:2 }}>Add items you want — we'll alert you when they go on sale</p>
              </div>
              <button onClick={checkWishlistSales} disabled={wishlistChecking||wishlist.length===0} style={{ background:"#2d5a1b", color:"#fff", border:"none", borderRadius:10, padding:"8px 14px", fontSize:12, fontWeight:600, cursor:"pointer", opacity:wishlist.length===0?0.5:1 }}>
                {wishlistChecking?"🔍 Checking...":"🔍 Check for Sales"}
              </button>
            </div>
            ...
            <div className="card" style={{ padding:16, marginBottom:16 }}>
              ...
              <div style={{ display:"flex", gap:8, marginBottom:8 }}>
                <input className="input" placeholder="e.g. Coca-Cola case, lobster, ribeye steak..." value={wishlistInput} onChange={e=>setWishlistInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addToWishlist()} />
              ...
              <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                <span style={{ fontSize:13, color:"#7a7060" }}>Alert me when price drops below: $</span>
                <input type="number" className="input" placeholder="e.g. 8.99" value={wishlistTargetPrice} onChange={e=>setWishlistTargetPrice(e.target.value)} style={{ width:100, padding:"8px 12px", fontSize:13 }} />
                <span style={{ fontSize:12, color:"#aaa" }}>CAD (optional)</span>
              </div>
            </div>

            {wishlistError&&<div style={{ background:"#fde8e8", border:"1px solid #f5c0c0", borderRadius:10, padding:"12px 16px", marginBottom:12, color:"#c0392b", fontSize:14 }}>⚠️ {wishlistError}</div>}

            {wishlist.length===0?(
              <div style={{ textAlign:"center", padding:"48px 24px", color:"#bbb" }}>
                <div style={{ fontSize:48, marginBottom:12 }}>⭐</div>
                <p style={{ fontStyle:"italic", fontSize:15, marginBottom:8 }}>Your watchlist is empty.</p>
                <p style={{ fontSize:13, color:"#aaa" }}>Add items you want to buy when they go on sale — like a case of Coca-Cola or lobster.</p>
              </div>
            ):(
              <div>
                {wishlist.some(w=>w.on_sale)&&(
                  <div style={{ background:"#e8f5e0", border:"2px solid #3d8c23", borderRadius:14, padding:16, marginBottom:16 }}>
                    <p style={{ fontSize:14, fontWeight:700, color:"#2d5a1b", margin:"0 0 8px" }}>🎉 Items on sale now!</p>
                    {wishlist.filter(w=>w.on_sale).map((w,i)=>(
                      <div key={i} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:6 }}>
                        <span style={{ fontSize:14, color:"#2d5a1b", fontWeight:600 }}>✅ {w.name}</span>
                        <div style={{ display:"flex", gap:8, alignItems:"center" }}>
                          {w.current_price&&<span style={{ fontSize:14, fontWeight:700, color:"#2d5a1b" }}>${w.current_price?.toFixed(2)}</span>}
                          <button onClick={()=>addWishlistItemToCart(w)} style={{ background:"#2d5a1b", color:"#fff", border:"none", borderRadius:8, padding:"5px 12px", fontSize:12, fontWeight:600, cursor:"pointer" }}>Add to List</button>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                {wishlist.map((item)=>{
                  ...
                  const lastChecked = item.last_checked ? new Date(item.last_checked).toLocaleDateString() : null;
                  return (
                      ...
                      <div style={{ padding:"14px 16px", display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                        <div style={{ flex:1 }}>
                          <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:4 }}>
                            <span style={{ fontSize:16, fontWeight:700, color:isOnSale?"#2d5a1b":"#1a1a1a" }}>{isOnSale?"🔥 ":""}{item.name}</span>
                            {isOnSale&&<span style={{ fontSize:11, fontWeight:700, background:"#3d8c23", color:"#fff", padding:"2px 8px", borderRadius:99 }}>ON SALE</span>}
                          </div>
                          {item.target_price&&<p style={{ fontSize:12, color:"#7a7060", margin:"0 0 4px" }}>🎯 Target price: ${item.target_price?.toFixed(2)} CAD</p>}
                          {item.current_price&&<p style={{ fontSize:13, color:isOnSale?"#2d5a1b":"#7a7060", margin:"0 0 4px", fontWeight:isOnSale?700:400 }}>Current: ${item.current_price?.toFixed(2)} CAD</p>}
                          {result&&result.saleStore&&<p style={{ fontSize:12, color:"#3d8c23", margin:"0 0 4px" }}>📍 {result.saleStore}{result.saleEnds&&result.saleEnds!=="unknown"?" · Until "+result.saleEnds:""}</p>}
                          {result&&result.note&&<p style={{ fontSize:12, color:"#7a7060", margin:0, fontStyle:"italic" }}>{result.note}</p>}
                          {lastChecked&&<p style={{ fontSize:11, color:"#aaa", margin:"4px 0 0" }}>Last checked: {lastChecked}</p>}
                        </div>
                        <div style={{ display:"flex", flexDirection:"column", gap:6, marginLeft:12, alignItems:"flex-end" }}>
                          {isOnSale&&<button onClick={()=>addWishlistItemToCart(item)} style={{ background:"#2d5a1b", color:"#fff", border:"none", borderRadius:8, padding:"7px 12px", fontSize:12, fontWeight:600, cursor:"pointer", whiteSpace:"nowrap" }}>+ Add to List</button>}
                          <button onClick={()=>removeFromWishlist(item.id)} style={{ background:"none", border:"1px solid #f5c0c0", borderRadius:8, padding:"5px 10px", fontSize:11, color:"#c0392b", cursor:"pointer" }}>Remove</button>
                        </div>
                      </div>
                    </div>
                  );
                ...
                <div style={{ background:"#f7f4ef", borderRadius:12, padding:14, marginTop:8 }}>
                  <p style={{ fontSize:12, color:"#7a7060", margin:0 }}>💡 Tap "Check for Sales" to search current flyers and prices for all your watchlist items. Items matching your target price will be highlighted.</p>
                </div>
              </div>
            )}
          </div>
...
function DishCard({ dish, onClick }) {
  ...
  return (
    <div className="dish-card" onClick={onClick} style={{ marginBottom:12 }}>
