...
export default function App() {
  ...
  const findPrices = async () => {
    ...
    // Use member's postal if they have one, otherwise fetch household owner's postal
    ...
    const budget = profile?.budget || (household ? (await supabase.from("profiles").select("budget").eq("id", household.owner_id).single()).data?.budget : 200) || 200;
    try {
      const res = await fetch("/api/prices", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ mode: "prices", items: items.map(i => i.name), postal, budget }),
      });
      const data = await res.json();
      if (data.error) throw new Error(data.error);
      const raw = data.text || "";
              ...
              <div>
                ...
                {activeMealCat === "All" ? (
                  Object.entries(dishesByCategory).map(([cat, catDishes]) => {
                    ...
                    return (
                      <div key={cat}>
                        ...
                        {catDishes.map((dish, i) => <DishCard key={i} dish={dish} onClick={() => openDish(dish)} saleItems={saleItems} />)}
                      </div>
                    );
                  })
                ) : (
                  filteredDishes.length === 0
                    ...
                    : filteredDishes.map((dish, i) => <DishCard key={i} dish={dish} onClick={() => openDish(dish)} saleItems={saleItems} />)
                )}
...
function DishCard({ dish, onClick, saleItems = [] }) {
  const meta = MEAL_CATEGORY_META[dish.mealCategory] || { icon:"🍽️", color:"#666", bg:"#f0f0f0" };
  // Check how many ingredients are on sale
  const saleIngredients = dish.ingredients ? dish.ingredients.filter(ing =>
    saleItems.some(saleItem =>
      ing.name.toLowerCase().includes(saleItem.toLowerCase()) ||
      saleItem.toLowerCase().includes(ing.name.toLowerCase().split(" ")[0])
    )
  ) : [];
  const hasSale = saleIngredients.length > 0;
  return (
    <div className="dish-card" onClick={onClick} style={{ marginBottom:12, border: hasSale ? "2px solid #3d8c23" : "1px solid var(--border)" }}>
