...
export default function App() {
  ...
  const checkWishlistSales = async () => {
    ...
    const prefix = profile.postal.replace(/\s/g, "").slice(0, 3).toUpperCase();
    const cityMap = { "L6C":"Markham","L6B":"Markham","L6E":"Markham","M1B":"Scarborough","M1P":"Scarborough","M4A":"East York","M5A":"Toronto","M5V":"Toronto","L4B":"Richmond Hill","L4J":"Thornhill","L5A":"Mississauga","L6P":"Brampton","L6R":"Brampton" };
    const cityName = cityMap[prefix] || "Toronto";
    try {
      const newResults = {};
      for (const item of wishlist) {
        const text = await callAI(
          "You are a Canadian grocery deal finder. Return ONLY valid JSON, no markdown.",
          'Is "' + item.name + '" on sale this week at any grocery store near ' + cityName + ' Ontario Canada? Check Loblaws, Walmart, No Frills, FreshCo, Food Basics. Return ONLY this JSON: {"currentPrice":0.00,"regularPrice":0.00,"onSale":false,"saleStore":"Store name","saleEnds":"date or unknown","savings":0.00,"note":"brief deal note"}'
        );
  ...
  const scanReceipt = async (file) => {
    ...
    try {
      ...
      const text = await callAI(
        "You are a receipt scanner. Extract purchased item names from receipts. Return ONLY a JSON array of item name strings.",
        "Extract all grocery item names from this receipt image (base64 encoded). Return ONLY a JSON array like: [\"milk\",\"bread\",\"eggs\"]. Image data: " + base64.substring(0, 1000)
      );
      const clean = text.replace(/```json|```/g, "").trim();
      const match = clean.match(/\[[\s\S]*\]/);
      ...
      items.forEach(item => {
        ...
        const found = purchasedItems.some(p => itemName.includes(p) || p.includes(itemName.split(" ")[0]));
    ...
    } catch (e) { setReceiptError("Could not scan receipt. Try a clearer photo."); }
  ...
  const findPrices = async () => {
    ...
    try {
      const res = await fetch("/api/prices", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ mode: "prices", items: items.map(i => i.name), postal: profile.postal, budget }),
      });
      const data = await res.json();
      if (data.error) throw new Error(data.error);
      const raw = data.text || "";
        ...
        {tab === "wishlist" && (
          <div className="fade-in">
            ...
            <div className="card" style={{ padding:16, marginBottom:16 }}>
              ...
              <div style={{ display:"flex", gap:8, marginBottom:10 }}>
                <input className="input" placeholder="e.g. Coca-Cola 24 pack, lobster, ribeye steak..." value={wishlistInput} onChange={e => setWishlistInput(e.target.value)} onKeyDown={e => e.key === "Enter" && addToWishlist()} />
              ...
              <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                <span style={{ fontSize:13, color:"#7a7060", whiteSpace:"nowrap" }}>Alert below: $</span>
                <input type="number" className="input" placeholder="e.g. 8.99" value={wishlistTargetPrice} onChange={e => setWishlistTargetPrice(e.target.value)} style={{ maxWidth:120, padding:"8px 12px", fontSize:13 }} />
              ...
              <div>
                {wishlist.some(w => w.on_sale) && (
                  <div style={{ background:"#e8f5e0", border:"2px solid #3d8c23", borderRadius:14, padding:16, marginBottom:16 }}>
                    <p style={{ fontSize:14, fontWeight:700, color:"#2d5a1b", margin:"0 0 10px" }}>🎉 Items on sale now!</p>
                    ...
                    <div key={item.id} className="card" style={{ marginBottom:12, overflow:"hidden", border:isOnSale?"2px solid #3d8c23":"1px solid #e2dbd0" }}>
                      <div style={{ padding:"14px 16px", display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                        <div style={{ flex:1 }}>
                          ...
                          {item.target_price && <p style={{ fontSize:12, color:"#7a7060", margin:"0 0 3px" }}>🎯 Target: ${item.target_price?.toFixed(2)} CAD</p>}
                          {item.current_price && <p style={{ fontSize:13, color:isOnSale?"#2d5a1b":"#7a7060", margin:"0 0 3px", fontWeight:isOnSale?700:400 }}>Current: ${item.current_price?.toFixed(2)} CAD</p>}
                          {result?.saleStore && <p style={{ fontSize:12, color:"#3d8c23", margin:"0 0 3px" }}>📍 {result.saleStore}{result.saleEnds && result.saleEnds !== "unknown" ? " · Until " + result.saleEnds : ""}</p>}
                          {result?.note && <p style={{ fontSize:12, color:"#7a7060", margin:0, fontStyle:"italic" }}>{result.note}</p>}
                ...
                <div style={{ background:"#f7f4ef", borderRadius:12, padding:14, marginTop:8 }}>
                  <p style={{ fontSize:12, color:"#7a7060", margin:0 }}>💡 Tap "Check Sales" to search current flyers and prices. Items at or below your target price will be highlighted.</p>
                </div>
              </div>
            )}
          </div>
