                    ...
                    <div key={item.id} className="card" style={{ marginBottom:12, overflow:"hidden", border:isOnSale?"2px solid #3d8c23":"1px solid #e2dbd0" }}>
                      <div style={{ padding:"14px 16px", display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                        <div style={{ flex:1 }}>
                          ...
                          {result?.saleStore && <p style={{ fontSize:12, color:"#3d8c23", margin:"0 0 3px" }}>🏪 {result.saleStore}{result.saleEnds && result.saleEnds !== "unknown" ? " · Sale until " + result.saleEnds : ""}</p>}
                          {result?.address && <p style={{ fontSize:12, color:"#7a7060", margin:"0 0 2px" }}>📍 {result.address}</p>}
                          {result?.hours && <p style={{ fontSize:12, color:"#7a7060", margin:"0 0 3px" }}>🕐 {result.hours}</p>}
