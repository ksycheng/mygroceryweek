export default async function handler(req, res) {
  ...
  if (mode === "search" && items && postal && serperKey) {
    try {
      ...
      // 8 searches per item across 10+ stores, ALL in parallel
      const [itemResults, storeResults] = await Promise.all([
        Promise.all(cleanedItems.map(item => Promise.all([
          ...
          webSearch('"' + item + '" price "Value Mart" OR "Giant Tiger" OR "Super C" Canada 2026', serperKey),
          webSearch('"' + item + '" weekly flyer sale Ontario Canada grocery store this week', serperKey),
          webSearch(item + ' grocery store price Ontario Canada 2026', serperKey),
          webSearch(item + ' price per kg OR per lb OR per litre Canada grocery store 2026', serperKey),
        ]))),
        Promise.all([
          webSearch('Walmart Loblaws "No Frills" Costco store address hours ' + city + ' Ontario', serperKey),
          webSearch('FreshCo "Food Basics" Metro Sobeys store address hours ' + city + ' Ontario', serperKey),
      ...
      cleanedItems.forEach((item, idx) => {
        ...
        itemSnippets[item] = combined.map(r => (r.title + ": " + r.snippet).substring(0, 200)).join("\n");
      });

      return res.status(200).json({
        itemSnippets,
        storeSnippets: storeResults.flat().slice(0, 12).map(r => (r.title + ": " + r.snippet).substring(0, 200)).join("\n"),
        city,
        cleanedItems,
  ...
  if (mode === "wishlist" && itemName && postal && serperKey) {
    try {
      ...
      const aiResponse = await callAI(
        "Extract grocery prices from web search results. Only report prices explicitly found. Return ONLY valid JSON.",
        'Find current price of "' + itemName + '" at local grocery stores near ' + city + ', Ontario from these search results:\n\n' + snippets + '\n\nReturn ONLY this JSON: {"currentPrice":0.00,"regularPrice":0.00,"onSale":false,"saleStore":"exact store name","address":"store street address in ' + city + ' ON if found in results","hours":"store hours if found in results","saleEnds":"date or unknown","savings":0.00,"note":"specific product and price e.g. Coca-Cola 12pk $9.97 at No Frills"}',
