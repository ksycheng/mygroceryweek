    ...
    try {
      ...
      const [itemResults, storeResults] = await Promise.all([
        Promise.all(cleanedItems.map(item => Promise.all([
          ...
          webSearch('"' + item + '" price "valumart" OR "giant tiger" canada 2026', serperKey),
  ...
  if (mode === "wishlist" && itemName && postal && serperKey) {
    try {
      ...
      const aiResponse = await callAI(
        ...
        'Find current price of "' + itemName + '" near ' + city + ', Ontario from these search results:\n\n' + snippets + '\n\nReturn ONLY: {"currentPrice":0.00,"regularPrice":0.00,"onSale":false,"saleStore":"store name","saleEnds":"date or unknown","savings":0.00,"note":"specific product and price found"}',
