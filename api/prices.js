...
{"currentPrice":0.00,"regularPrice":0.00,"onSale":false,"saleStore":"exact store name from results or null","saleEnds":"date from results or unknown","savings":0.00,"note":"exact quote from search result e.g. found on walmart.ca: Coca-Cola 12pk $9.97"}`,
  ...
  if (mode === "prices" && items && postal && serperKey) {
    try {
      ...
      const top8 = itemList.slice(0, 8).join(" ");
      const [walmartR, loblawsR, nofrillsR, storeR] = await Promise.all([
        webSearch(top8 + ' price walmart.ca Canada grocery 2026', serperKey),
        webSearch(top8 + ' price loblaws.ca Canada grocery 2026', serperKey),
        webSearch(top8 + ' price "no frills" OR "food basics" OR "freshco" Canada grocery 2026', serperKey),
        webSearch('Loblaws Walmart "No Frills" FreshCo address hours ' + city + ' Ontario', serperKey),
      ]);

      const allSnippets = [...walmartR, ...loblawsR, ...nofrillsR];
      const priceSnippets = allSnippets.slice(0, 15).map(r => r.title + ": " + r.snippet).join("\n");
      const storeSnippets = storeR.slice(0, 5).map(r => r.title + ": " + r.snippet).join("\n");
...
async function callOpenRouter(system, prompt, maxTokens = 2048) {
  ...
  // Use openrouter/free - auto-selects from all available free models
  try {
    ...
    const data = await response.json();
    if (data.error) { console.error("OpenRouter error:", JSON.stringify(data.error)); return null; }
    const text = data?.choices?.[0]?.message?.content;
    if (text && text.trim()) { console.log("OpenRouter success"); return text; }
  } catch(e) { console.error("OpenRouter exception:", e.message); }
  return null;
}
