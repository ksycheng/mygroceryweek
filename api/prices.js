...
async function callAI(system, prompt, groqKey, geminiKey, maxTokens = 2048, fast = false) {
  // Try Groq first
  if (groqKey) {
    const result = await callGroq(system, prompt, groqKey, maxTokens, fast);
    if (result && result.trim() !== "") {
      console.log("Used Groq");
      return result;
    }
    console.log("Groq empty/failed, trying Gemini fallback...");
  }
  // Fall back to Gemini
  if (geminiKey) {
    const result = await callGemini(system, prompt, geminiKey, maxTokens);
    if (result && result.trim() !== "") {
  ...
  if (claudeKey) {
    const result = await callClaude(system, prompt, claudeKey, maxTokens);
    if (result && result.trim() !== "") {
      console.log("Used Claude fallback");
      return result;
    }
  }
  // Fall back to OpenRouter (free models)
  const orResult = await callOpenRouter(system, prompt, maxTokens);
  if (orResult && orResult.trim() !== "") {
