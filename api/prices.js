export default async function handler(req, res) {
  ...
  if (mode === "prices" && items && postal && serperKey) {
    try {
      ...
      // Clean items and search each one individually
      ...
      for (let i = 0; i < itemList.length; i += batchSize) {
        ...
        const batchResults = await Promise.all(
          batch.map(item => {
            ...
            return webSearch(cleanName + ' price canada grocery walmart loblaws nofrills 2026', serperKey);
          })
        );
        batch.forEach((item, idx) => {
          const cleanName = cleanItemName(item);
          allItemSnippets[cleanName] = batchResults[idx].map(r => r.title + ": " + r.snippet).join("\n");
        });
      }
...
ITEMS NEEDED: ${itemList.map(i => cleanItemName(i)).join(", ")}
...
- IMPORTANT: Calculate totals by adding up ALL found prices. totalCAD and subtotals MUST be the sum of all item prices, not zero
...
function cleanItemName(item) {
  // Strip quantities and measurements - just keep the ingredient name
  return item
    .replace(/^\d+x?\s+/i, '')           // remove leading numbers like "2x", "3 "
    .replace(/^\d+\s*\/\s*\d+\s+/i, '')  // remove fractions like "1/2 "
    .replace(/^[\d.]+\s*(cups?|tbsp|tsp|tablespoons?|teaspoons?|pounds?|lbs?|kg|g|oz|ml|l|liters?|litres?|slices?|cloves?|pieces?|cans?|boxes?|bags?|bunches?|heads?|stalks?|sheets?|inches?|quarts?)\s+/i, '')
