export default async function handler(req, res) {
  ...
  if (mode === "search" && items && postal && serperKey) {
    try {
      ...
      // 3 focused searches per item — most effective, stays well within limits
      // Process in batches of 5 items to avoid overwhelming Serper
      const BATCH = 5;
      const itemSnippets = {};

      for (let i = 0; i < cleanedItems.length; i += BATCH) {
        const batch = cleanedItems.slice(i, i + BATCH);
        const batchResults = await Promise.all(batch.map(item => Promise.all([
          webSearch('"' + item + '" price Walmart OR Loblaws OR "No Frills" OR "Food Basics" Canada grocery 2026', serperKey),
          webSearch('"' + item + '" price Costco OR Metro OR Sobeys OR FreshCo Canada grocery 2026', serperKey),
          webSearch(item + ' grocery price Ontario Canada flyer sale this week 2026', serperKey),
        ])));
        batch.forEach((item, idx) => {
          const combined = batchResults[idx].flat().slice(0, 12);
